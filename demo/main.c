...
void scene_init(struct scene *s, enum scene_type type, struct gkab_arena *arena) {
    ...
    case ST_TEAPOT:
        ...
        model_load(&s->as.model.model, "assets/teapot.obj", "assets/grey.png", glm_rad(0.0f), false, (vec3s) { 0.05f, 0.05f, 0.05f }, (vec3s) { 0.0f, 0.0f, 0.0f }, arena);
        break;
    case ST_CESSNA:
        camera_view(&s->camera, (vec3s) { 0.0f, 0.0f, 12.0f }, (vec3s) { 0.0f, 0.0f, 0.0f });
        model_load(&s->as.model.model, "assets/cessna.obj", "assets/grey.png", glm_rad(0.0f), true, (vec3s) { 0.3f, 0.3f, 0.3f }, (vec3s) { 0.0f, 0.0f, 0.0f },  arena);
        break;
    case ST_MINICOOPER: 
        camera_view(&s->camera, (vec3s) { 0.0f, 0.0f, 12.0f }, (vec3s) { 0.0f, 0.0f, 0.0f });
        model_load(&s->as.model.model, "assets/minicooper.obj", "assets/grey.png", glm_rad(0.0f), true, (vec3s) { 0.05f, 0.05f, 0.05f }, (vec3s) { 0.0f, 10.0f, -20.0f },  arena);
        break;
    case ST_TEXTURE:
        camera_view(&s->camera, (vec3s) { 0.0f, 0.0f, 12.0f }, (vec3s) { 0.0f, 0.0f, 0.0f });
        for (int i = 0; i < 1; i++) {
    ...
    case ST_MARS: {
        ...
        {
            FILE *f = fopen("assets/mars.bin", "rb");
...
#define BUCKET_WIDTH (65536 / BUCKET_COUNT )
...

        {
            ...
            s->as.mars.colormap = stbi_load("assets/cm2_orange.png", &bg_width, &bg_height, &bg_channels, 0);
...
void scene_draw(struct scene *s, struct renderer *r, float rads) {
    renderer_begin_frame(r, g_app.background);
...
int main(int argc, char **argv) {
    ...
    app_init(&g_app, window.pixels, window.surface_width, window.surface_height, "assets/background.png");
...
int main(void) {
    ...
    app_init(&g_app, pixels, WIDTH, HEIGHT, "assets/background.png"); // "/cessna.obj", "/grey.png");
    g_left = false;
    g_right = false;
